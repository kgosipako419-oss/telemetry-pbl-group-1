...
Column layout — filtered:
  timestamp | {col}_baseband | {col}_{scheme}_output | ... | current_state_binary

Column layout — features:
  segment_start | segment_end | segment_idx | {col}_baseband | {col}_{scheme}_output | ... | current_state_binary
"""
...
# Metadata columns that must be carried through from features files
...
def _build_scheme_dict(df, cols, index_col, scheme):
    ...
    out["current_state_binary"] = df["current_state_binary"] \
    ...
    for station in STATIONS:
        ...
        if os.path.exists(fp):
            ...
            for scheme in ["AM", "FM", "ASK", "FSK", "PSK"]:
                ...
                print(f"  [filtered][{scheme}]  {rows} rows × {cols} cols"
        ...
        if os.path.exists(xp):
            ...
            for scheme in ["AM", "FM", "ASK", "FSK", "PSK"]:
                ...
                print(f"  [features][{scheme}]  {rows} rows × {cols} cols"
                      f"  -> {os.path.basename(path)}")
                total += 1
